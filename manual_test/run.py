...
from base.utils import standalone_main, makedir_p
...
class TestRunner:
    ...
    def init(self):
        ...
        if not (self.options.name and self.options.api):
    ...
    def download(self, api, model):
        ...
        if self.options.download_dir and os.path.exists(filename):
            print('Loading from local file: %s' % filename)
            try:
                data = json.load(open(filename, 'r'))
                return data
            except Exception as e:
                print('Loading from local file %s: %s' % (filename, e))

        print('Downloading from api %s' % api)
    ...
    def run_testcase(self, tc, prefix=''):
        ...
        try:
            ...
            output = self.get_data_from_api_endpoint(tc['input'], tc['config'])
        except Exception as e:
        ...
        if not errors:
            if output == None:
                ...
                try:
                    ...
                    tc_output['line_items'] = self.remove_uuid_from_line_item(tc_output.get("line_items", []))
                    output['line_items'] = self.remove_uuid_from_line_item(output.get("line_items", []))
    ...
    def get_data_from_api_endpoint(self, input, config):
        """
        :return:
        """
        url = config.get("url")
        payload = json.dumps(input.get('json').get('data'))
        headers = {
    ...
    def remove_uuid_from_line_item(self, line_item):
        """Remove uuid from line item data
        :param line_item:
        :return:
        """
